...
export default function NewYorkElectricity() {
  ...
  const stateProviders = providersInState("NY").slice(0, 6);
  const renewableHere = renewableProvidersInState("NY");
  ...
  const stateData = {
    ...
    faqs: [
      ...
      },
      {
        ...
        answer: `Yes. ${stateMarket?.renewablePlans ?? 0} of the ${stateMarket?.plans ?? 0} New York plans we track are backed by 100% renewable energy${renewableHere.length ? `, from suppliers including ${renewableHere.slice(0, 3).join(", ")}` : ""}. Renewable supply in a competitive market is normally matched with renewable energy certificates rather than a dedicated line to a wind farm.`
      }
    ]
  };
