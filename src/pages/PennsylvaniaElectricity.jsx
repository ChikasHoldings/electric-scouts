...
export default function PennsylvaniaElectricity() {
  ...
  const stateProviders = providersInState("PA").slice(0, 6);
  const renewableHere = renewableProvidersInState("PA");
  ...
  const stateData = {
    ...
    faqs: [
      ...
      },
      {
        ...
        answer: `Yes. ${stateMarket?.renewablePlans ?? 0} of the ${stateMarket?.plans ?? 0} Pennsylvania plans we track are backed by 100% renewable energy${renewableHere.length ? `, from suppliers including ${renewableHere.slice(0, 3).join(", ")}` : ""}. Renewable supply in a competitive market is normally matched with renewable energy certificates rather than a dedicated line to a wind farm.`
      }
    ]
  };
            ...
            <Card className="border-2 hover:shadow-xl transition-all">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-gray-600">
                  Suppliers price the same Pennsylvania power differently. We show what each one
